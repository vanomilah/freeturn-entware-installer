...
ARGS="-connect $PEER -listen 127.0.0.1:51820 -obf-profile $OBF -obf-key $KEY"
...
if [ "$REQUEST_METHOD" = "POST" ]; then
    ...
    if [ -f "$CONFIG" ]; then
        sed -i "s|-connect [^ ]*|-connect $NEW_PEER|g" "$CONFIG"
        sed -i "s|-obf-profile [^ ]*|-obf-profile $NEW_OBF|g" "$CONFIG"
...
<head>
    ...
    <style>
        ...
        textarea { width: 100%; padding: 10px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; font-family: monospace; height: 80px; resize: vertical; }
    ...
    <div class="container">
        ...
        <script>
            if (window.location.search.includes('success=1')) {
                document.write('<div class="status">✅ Настройки клиента обновлены и применены!</div>');
            }
        </script>

        <div class="info">
            <b>Текущий сервер:</b> $CUR_PEER<br>
            <b>Обфускация:</b> $CUR_OBF
        </div>

        <label>Вставьте новую ссылку freeturn://</label>
        <textarea id="linkInput" oninput="decodeLink()" placeholder="freeturn://..."></textarea>
        
        <div id="preview" style="display:none; margin-top: 15px; font-size: 14px; color: #0056b3;">
            <b>Новый сервер:</b> <span id="newPeer"></span>
        </div>
        ...
        <form method="POST" action="client.cgi">
            ...
            <button type="submit" id="applyBtn" disabled>Применить и перезапустить</button>
        </form>

        <div id="wgBlock" style="display:none; margin-top: 25px; padding-top: 15px; border-top: 2px dashed #ccc;">
            <b style="color: #d32f2f;">Внимание!</b> Не забудьте обновить настройки в клиенте WireGuard:
    ...
    <script>
        function decodeLink() {
            ...
            if (!input.startsWith("freeturn://")) {
                btn.disabled = true; preview.style.display = 'none'; wgBlock.style.display = 'none'; return;
            }
            ...
            try {
                ...
                btn.disabled = true; preview.style.display = 'none'; wgBlock.style.display = 'none';
            }
        }
...
echo "👉 Страница обновления клиента:"
