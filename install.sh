...
if echo "$ENTWARE_ARCH" | grep -q "mipsel"; then
    FT_ARCH="mipsle-softfloat"
elif echo "$ENTWARE_ARCH" | grep -q "mips"; then
    FT_ARCH="mips-softfloat"
elif echo "$ENTWARE_ARCH" | grep -q "aarch64\|arm64"; then
    FT_ARCH="arm64"
elif echo "$ENTWARE_ARCH" | grep -q "armv7"; then
    FT_ARCH="armv7"
elif echo "$ENTWARE_ARCH" | grep -q "x86_64"; then
    FT_ARCH="amd64"
else
    ARCH=$(uname -m)
    case "$ARCH" in
...
echo "✅ Выбрана архитектура: $FT_ARCH"
...
if [ -z "$LATEST_VERSION" ]; then
    echo "⚠️ Не удалось получить версию по API. Используем v1.7.3 по умолчанию."
    LATEST_VERSION="v1.7.3"
fi

FILE_NAME="server-linux-${FT_ARCH}"
PRIMARY_URL="https://github.com/samosvalishe/free-turn-proxy/releases/download/${LATEST_VERSION}/${FILE_NAME}"
...
# Попытка скачать напрямую, если фейл — скачиваем через зеркало
if ! curl -4 -kL --connect-timeout 8 -o /opt/bin/freeturn-server "$PRIMARY_URL"; then
    echo "⚠️ Прямое скачивание не удалось. Пробуем через зеркало..."
    curl -4 -kL -o /opt/bin/freeturn-server "$MIRROR_URL"
fi
...
if [ -s "/opt/bin/freeturn-server" ]; then
    ...
    echo "❌ Ошибка скачивания бинарника!"
    rm -f /opt/bin/freeturn-server
    exit 1
fi

# --- 3. Настройка окружения, папок и генерация ключей ---
mkdir -p /opt/var/run /opt/tmp
chmod 777 /opt/tmp
...
EXTERNAL_IP=$(curl -4 -ks http://icanhazip.com 2>/dev/null)
if [ -z "$EXTERNAL_IP" ]; then
    EXTERNAL_IP=$(curl -4 -ks http://api.ipify.org 2>/dev/null)
fi
...
chmod 755 /opt/etc/init.d/S99vk-turn-server
sed -i 's/\r$//' /opt/etc/init.d/S99vk-turn-server

# --- 5. Настройка Web-сервера (Чистый, стабильный и бесконфликтный метод) ---
echo "🌐 Генерация конфигурационного файла веб-сервера..."
cat << 'ETALON_EOF' > /opt/etc/lighttpd/lighttpd.conf
server.modules = (
    "mod_cgi",
    "mod_access"
)
...
index-file.names            = ( "index.html", "generator.cgi" )
...
if [ "$REQUEST_METHOD" = "POST" ]; then
    ...
    if echo "$POST_DATA" | grep -q "action=update_config"; then
        ...
        NEW_PORT=$(echo "$POST_DATA" | sed -n 's/.*port=\([^&]*\).*/\1/p')
...
<head>
    ...
    <style>
        ...
        input, select, textarea { width: 100%; padding: 8px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; font-family: monospace; }
    ...
    <div class="container">
        ...
        <div style="margin-top: 20px; border-top: 1px solid #ccc; padding-top: 10px;">
            <form action="generator.cgi" method="POST">
                <button type="submit" name="action" value="restart" style="background: #ffc107; color: black; width: 100%;">Перезапустить FreeTurn</button>
            </form>
        </div>
        ...
        <form action="generator.cgi" method="POST" style="margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; background: #f9f9f9;">
            <input type="hidden" name="action" value="update_config">
            <div class="row">
                <div class="col"><label>Obfuscation</label><input type="text" name="obf" value="$FT_OBF"></div>
                <div class="col"><label>Port</label><input type="number" name="port" value="$FT_PORT"></div>
            </div>
            <button type="submit" style="background: #6c757d; margin-top: 10px;">Сохранить настройки и перезапустить</button>
        </form>
