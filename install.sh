...
# Пытаемся спросить у opkg реальную архитектуру Entware
...
if echo "$ENTWARE_ARCH" | grep -q "mipsel"; then
    ...
    # Резервный вариант, если opkg недоступен
    ARCH=$(uname -m)
    case "$ARCH" in
...
LATEST_VERSION=$(curl -kLs "$API_URL" | grep '"tag_name":' | awk -F '"' '{print $4}')
...
curl -kL -o /opt/bin/freeturn-server "$DOWNLOAD_URL"
...
EXTERNAL_IP=$(curl -ks http://api.ipify.org 2>/dev/null)
